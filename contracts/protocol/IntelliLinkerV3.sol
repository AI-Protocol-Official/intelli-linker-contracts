...
contract IntelliLinkerV3 is UpgradeableAccessControl {
	...
	function link(uint96 personalityId, address targetContract, uint256 targetId) public virtual {
		...
		IntelligentNFTv2(iNftContract).mint(
			nextId++,
			linkPrice - linkFee,
			personalityContract,
			personalityId,
	...
	function updateLinkPrice(uint96 _linkPrice, uint96 _linkFee, address _feeDestination) public virtual {
		...
		require(
			(_linkFee == 0 && _feeDestination == address(0)) || (_linkFee >= 1e12 && _feeDestination != address(0)),
	...
	function isWhitelisted(
		...
		view
		virtual
		returns (bool allowedForLinking, bool allowedForUnlinking, bool forbiddenForLinking, bool forbiddenForUnlinking)
	{
	...
	function isAllowedForUnlinking(address targetContract) public view virtual returns (bool) {
		...
		// evaluate the result based on the values read
		return
			!forbiddenForUnlinking && (allowedForUnlinking || isFeatureEnabled(FEATURE_ALLOW_ANY_NFT_CONTRACT_FOR_UNLINKING));
	}
}
